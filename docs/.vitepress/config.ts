...
export default defineConfig({
	...
	head: [
		...
		[
			...
			{},
			`
			if (typeof window !== 'undefined' && window.location.pathname.replace(/\\/$/, '') === '/mcp-telegram' || window.location.pathname === '/mcp-telegram/') {
				const lang = navigator.language || navigator.userLanguage || 'en';
				const lc = lang.toLowerCase();
				if (lc.startsWith('ru')) {
					window.location.replace('/mcp-telegram/ru/');
				} else if (lc.startsWith('zh')) {
					window.location.replace('/mcp-telegram/zh/');
				}
			}
			`,
		],
